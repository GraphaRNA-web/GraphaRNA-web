...
class PostRnaDataTests(APITestCase):
    def setUp(self) -> None:
        self.client: APIClient = APIClient()
        ...
        self.valid_data: Dict[str, Any] = {
            ...
            "enable_dpm": True,  # Added the new DPM parameter
        }
    ...
    def test_valid_post_with_dpm_disabled(self) -> None:
        """
        Tests if the backend properly handles the new enable_dpm flag when set to False/false.
        """
        data = self.valid_data.copy()
        data["job_name"] = "job-test-dpm-disabled"
        data["enable_dpm"] = "false"  # Emulate frontend form data parsing

        response: Response = self.client.post(self.url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(response.data["success"])
        
        # Verify the job was created and has enable_dpm properly evaluated
        self.assertTrue(Job.objects.filter(job_name="job-test-dpm-disabled").exists())
        # Assuming you want to verify the model property directly:
