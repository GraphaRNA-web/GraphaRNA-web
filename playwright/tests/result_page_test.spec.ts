...
test.describe("Result Page Tests, Finished Job, Multiple Results", () => {
  ...
  test("should open result page and show results using pageManager", async ({ pageManager, frontendUrl }) => {
    ...
    await pageManager.onResultsPage.structure2D.toHaveScreenshot("2d_image.png");
    await pageManager.onResultsPage.arcDiagram.toHaveScreenshot("arc_diagram.png");
    // there are two canvas elements rendered for 3D structure, explicitly select the first one
    await expect(pageManager.onResultsPage.structure3D.element().first()).toHaveScreenshot("3d_image.png");
